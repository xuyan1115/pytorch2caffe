...
layer_dict_2 = {'ConvNdBackward': 'Convolution',
              'ThresholdBackward': 'ReLU',
              'MaxPool2dBackward': 'Pooling', # MaxPool2dBackward for torch 0.2.0
              'AvgPool2dBackward': 'Pooling', # AvgPool2dBackward for torch 0.2.0
              'DropoutBackward': 'Dropout',
              'AddmmBackward': 'InnerProduct',
              'BatchNormBackward': 'BatchNorm',
              'AddBackward': 'Eltwise',   # AddBackward for torch 0.2.0
...
layer_dict_3 = {'ConvNdBackward': 'Convolution',
              'ThresholdBackward': 'ReLU',
              'MaxPool2DBackward': 'Pooling', # MaxPool2dBackward for torch 0.2.0
              'AvgPool2DBackward': 'Pooling', # AvgPool2dBackward for torch 0.2.0
              'DropoutBackward': 'Dropout',
              'AddmmBackward': 'InnerProduct',
              'BatchNormBackward': 'BatchNorm',
              'AddBackward1': 'Eltwise',   # AddBackward for torch 0.2.0
              'ViewBackward': 'Reshape',
              'ConcatBackward': 'Concat',
              'UpsamplingNearest2d': 'Deconvolution',
              'UpsamplingBilinear2d': 'Deconvolution',
...
layer_dict = layer_dict_3 if float(torch.__version__[:3]) >= 0.3 \
                    else layer_dict_2
...
def pytorch2prototxt(input_var, output_var):
    ...
    def add_layer(func):
        ...
        if parent_type == 'MulBackward':
        ...
        elif parent_type == 'MaxPool2dBackward' or parent_type == 'MaxPool2DBackward':
            pooling_param = OrderedDict()
            pooling_param['pool'] = 'MAX'
            print(func.next_functions)
            if func.kernel_size[0] != func.kernel_size[1]:
        ...
        elif parent_type == 'AvgPool2dBackward' or parent_type == 'AvgPool2DBackward':
            pooling_param = OrderedDict()
            ...
            parent_top = parent_bottoms[0]
        elif parent_type == 'AddBackward' or parent_type == 'AddBackward1':
            ...
            for idx, value in enumerate(func.index):
                if value != slice(None):
                    break
            slice_param['axis'] = idx
            if value.stop != func.input_size[idx]:
                slice_param['slice_point'] = [value.stop]
    ...
    slice_list = []
    bottom_tmp = {}
    for idx, l in enumerate(net_info['layers']):
        if l['type'] == 'Slice':
            if l['bottom'] in bottom_tmp.values():
                net_info['layers'].remove(l)
            else:
                bottom_tmp[idx] = l['bottom']
            slice_list.append(l)

    for k, v in bottom_tmp.iteritems():
        slice_layer = OrderedDict()
        for l in slice_list:
            if l['bottom'] == v:
                if len(slice_layer) == 0:
                    slice_layer['name'] = l['name']
                    slice_layer['type'] = l['type']
                    slice_layer['bottom'] = l['bottom']
                    slice_layer['top'] = [l['top']]
                    slice_layer['slice_param'] = deepcopy(l['slice_param'])
                else:
                    slice_layer['top'].append(l['top'])
                    if l['slice_param'].get('slice_point'):
                        slice_layer['slice_param']['slice_point'].extend(l['slice_param']['slice_point'])
        net_info['layers'][k] = deepcopy(slice_layer)
...
def plot_graph(top_var, fname, params=None):
    ...
    if isinstance(top_var, tuple):
        for idx in top_var:
            for var in idx:
                add_nodes(var.grad_fn)
    elif isinstance(top_var, list):
        for var in top_var:
            add_nodes(var.grad_fn)
    else:
